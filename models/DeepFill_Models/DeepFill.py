...
class Generator(nn.Module):
    def __init__(self, first_dim=32, isCheck=False, device=None, build_refine=True, detach_coarse=False, is_fg=False):
        super(Generator, self).__init__()
        self.isCheck = isCheck
        self.device = device
        self.build_refine = build_refine
        self.detach_coarse = detach_coarse

        if is_fg:
            self.stage_1 = CoarseNet(8, first_dim, device=device)
        else:
            self.stage_1 = CoarseNet(5, first_dim, device=device)
        self.stage_2 = RefinementNet(5, first_dim, device=device)

    def forward(self, masked_img, mask, small_mask, bg_img=None): # mask : 1 x 1 x H x W
        # border, maybe
        ...
        # stage1: predict coarse result
        if bg_img is None:
            stage1_input = torch.cat([masked_img, ones, ones*mask], dim=1)
        else:
            stage1_input = torch.cat([masked_img, ones, ones*mask, bg_img], dim=1)
        stage1_output, resized_mask = self.stage_1(stage1_input, mask)

        # stage2
        if self.build_refine:
            if self.detach_coarse:
                stage1_output = stage1_output.detach() # TODO: ? is it correct to freeze CoarseNetwork?

            new_masked_img = stage1_output*mask.clone() + masked_img.clone()*(1.-mask.clone())
            # new_masked_img.detach() # TODO:??
            ...
            stage2_output = None
            offset_flow = None
            return stage1_output, stage1_output, stage1_output



class CoarseNet(nn.Module):
    '''
    # input: B x 5 x W x H TODO:???
